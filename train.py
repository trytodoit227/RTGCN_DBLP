...
if __name__ == "__main__":
    ...
    parser.add_argument("--early_stop", type=int, default=50,#20
                        help="patient")
    ...
    parser.add_argument('--neg_sample_size', type=int, nargs='?', default=10,#10
                        help='# negative samples per positive')
    # Walk length for random walk sampling.
    parser.add_argument('--walk_len', type=int, nargs='?', default=20,
                        help='Walk length for random walk sampling')
    # Weight for negative samples in the binary cross-entropy loss function.
    parser.add_argument('--neg_weight', type=float, nargs='?', default=10,
                        help='Weightage for negative samples')
    
    parser.add_argument('--loss_weight', type=float, nargs='?', default=1,
                        help='Weightage for role loss')
    
    parser.add_argument('--learning_rate', type=float, nargs='?', default=0.008,
    ...
    parser.add_argument('--weight_decay', type=float, nargs='?', default=0.0003,#
                        help='Initial learning rate for self-attention model.')
    ...
    #load graph data
    graphs, adjs, feats, data_dblp = load_graphs(args.dataset)
    ...
    for epoch in range(args.epochs):
        ...
        for idx,feed_dict in enumerate(dataloader):
            # feed_dict = dataset.next_minibatch_feed_dict()
    ...
    emd_base_path = '/home/ynos/Desktop/RTGCN/RTGCN_DBLP3_nc/visualization'

    if task=='node classification':
        average_accs,temp_accs,average_aucs,temp_aucs=evaluate_node_classification(epochs_all_embeds[-1],labels,train_nodes)

        # y = save_embedding(epochs_all_embeds[-1], args.time_steps, emd_base_path)
